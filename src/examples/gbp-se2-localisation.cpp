...
int main() {
    ...
    auto randn = [&](double sigma) { return sigma * gauss(rng); };

    const Eigen::Vector3d odom_sigma(0.04, 0.04, 0.02);   // odometry noise (x, y, theta)
    const Eigen::Vector2d meas_sigma(0.10, 0.10);         // landmark-position noise (x, y in robot frame)

    // The true robot, and the factor graph that estimates it.
    LieGroup true_robot, last_keyframe;
    FactorGraph fg(0);
    int lid = 0;
    Key current_pose;
    std::vector<Key> pose_keys, landmark_keys;
    std::vector<Eigen::Vector2d> landmark_true;
    int timestep = 0;
